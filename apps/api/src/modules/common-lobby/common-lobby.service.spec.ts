...
describe('CommonLobbyService', () => {
  ...
  describe('joinRoom', () => {
    it('joins a new player', () => {
      const { room } = service.createRoom('Alice', 's1');
      const { room: joined } = service.joinRoom(room.code, 'Bob', 's2');
    ...
    it('reconnects a disconnected player', () => {
      const { room, sessionToken } = service.createRoom('Alice', 's1');
      service.markDisconnected('s1');
      const { room: rejoined } = service.joinRoom(room.code, 'Alice', 's1-new', sessionToken);
