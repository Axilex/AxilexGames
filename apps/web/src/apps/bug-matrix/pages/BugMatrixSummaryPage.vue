  ...
  <div class="min-h-screen flex flex-col">
    <header class="sticky top-0 z-10 bg-card border-b border-border px-6 py-4">
      <div class="max-w-5xl mx-auto flex items-center justify-between gap-4">
        <h1 class="text-lg font-bold text-foreground">🧠 Bug dans la Matrix — Fin de partie</h1>
      </div>
    </header>

    <main class="flex-1 max-w-3xl mx-auto w-full px-6 py-10 flex flex-col gap-8">
      <ol class="bg-card rounded-2xl border border-border p-6 flex flex-col gap-3">
        <li
          v-for="entry in store.finalRanking"
          :key="entry.pseudo"
          :class="[
            'flex items-center justify-between p-3 rounded-xl',
            entry.rank === 1 ? 'bg-amber-50 border border-amber-200' : 'border border-border',
          ]"
        >
          <div class="flex items-center gap-3">
            ...
            <span class="font-bold text-foreground">{{ entry.pseudo }}</span>
          </div>
