  ...
  <div class="bg-card rounded-2xl border border-border p-6 flex flex-col gap-4">
    ...
    <div class="flex items-center justify-between">
      <div class="flex flex-col gap-0.5">
        ...
        <h2 class="text-xl font-bold text-foreground">Les mots</h2>
      </div>
    </div>
    ...
    <ul v-if="words && words.length" class="flex flex-col gap-2">
      <li
        v-for="(w, i) in words"
        :key="i"
        class="flex items-center gap-3 rounded-xl border border-border bg-surface-muted px-4 py-2.5"
        :class="{
      ...
      >
        <span class="text-xs font-bold text-foreground-subtle w-5 shrink-0">{{ i + 1 }}.</span>
        <span class="flex-1 text-sm font-semibold text-foreground">{{ w }}</span>
      </li>
    </ul>
    <p v-else class="text-sm text-foreground-subtle">Aucun mot soumis.</p>
    ...
    <template v-if="showVoting">
      ...
      <p v-else-if="!canVote" class="text-sm text-foreground-muted text-center">
