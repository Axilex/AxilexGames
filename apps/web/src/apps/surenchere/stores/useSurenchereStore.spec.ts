...
function makeRoom(overrides: Partial<SurenchereRoomDTO> = {}): SurenchereRoomDTO {
  return {
    ...
    settings: { totalRounds: 5, startBid: 5, wordTimerSeconds: 60 },
    currentRound: 1,
    currentChallenge: {
      id: 'c1',
      category: 'Test',
...
describe('useSurenchereStore', () => {
  ...
  it('isChallengeChooser true when I am chooser and phase CHOOSING_CHALLENGE', () => {
    ...
    store.setRoom(
      makeRoom({
        ...
        challengeOptions: [
          { id: 'a', category: 'X', prompt: 'p', source: 'predefined' },
          { id: 'b', category: 'Y', prompt: 'p', source: 'predefined' },
          { id: 'c', category: 'Z', prompt: 'p', source: 'predefined' },
        ],
      }),
    );
  ...
  it('addRoundResult prepends to history', () => {
    ...
    const result: SurenchereRoundResult = {
      bidderSocketId: 's1',
      bidderPseudo: 'Alice',
      challenge: { id: 'c1', category: 'Test', prompt: 'p', source: 'predefined' },
      bid: 7,
      success: true,
      scoreDelta: 7,
      missingCount: 0,
