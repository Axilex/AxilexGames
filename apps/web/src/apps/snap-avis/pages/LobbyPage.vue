...
const canStart = computed(() => (store.room?.players.filter((p) => p.status === 'CONNECTED').length ?? 0) >= 2);
const connectedCount = computed(() => store.room?.players.filter((p) => p.status === 'CONNECTED').length ?? 0);
