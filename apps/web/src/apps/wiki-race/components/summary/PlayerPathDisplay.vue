<template>
  <div class="flex flex-col gap-2">
    <div class="flex items-center gap-2">
      ...
      <span v-else class="text-foreground-subtle text-lg">—</span>
      <span class="font-semibold text-foreground">{{ pseudo }}</span>
      <span class="text-foreground-subtle text-sm">{{ hopCount }} clic{{ hopCount !== 1 ? 's' : '' }}</span>
      <button
        v-if="path.length > 0"
        class="ml-auto text-foreground-subtle hover:text-amber-500 transition-colors p-1 rounded hover:bg-amber-50"
    ...
    <div class="flex flex-wrap items-center gap-1 pl-6">
      ...
      <template v-for="(step, i) in path" :key="i">
        <svg class="h-3 w-3 text-foreground-subtle" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <span v-if="path.length === 0" class="text-foreground-subtle text-xs italic">N'a pas bougé</span>
    </div>
  </div>
</template>
